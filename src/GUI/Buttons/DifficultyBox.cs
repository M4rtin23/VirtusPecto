...
namespace VirtusPecto.Desktop{
	public class DifficultyBox{
		...
		public void Draw(SpriteBatch batch){
			batch.Draw(BoxSprite, BoxPosition, color);
			batch.DrawString(Font, " " + Difficulty, BoxPosition, Color.White);
			if (isActivated){
				color = Color.LightGray;
				for (int i = 0; i < OptionsNumber; i++){
					batch.Draw(BoxSprite, new Vector2(BoxPosition.X, BoxPosition.Y + (1 + i) * 32), Colors[i]);
					batch.DrawString(Font, Options[i], new Vector2(BoxPosition.X, BoxPosition.Y + (i + 1) * 32), Color.White);
				}
			}else{
